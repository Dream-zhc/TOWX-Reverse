...
@interface TOWXV11AvatarCell : UIControl
...
@implementation TOWXV11AvatarCell {
...
- (void)setHighlighted:(BOOL)highlighted {
    ...
    if (highlighted) {
        self.pressAnimator = [[UIViewPropertyAnimator alloc] initWithDuration:0.07 curve:UIViewAnimationCurveEaseOut animations:^{
        ...
        self.pressAnimator = [[UIViewPropertyAnimator alloc] initWithDuration:0.18 timingParameters:timing];
        [self.pressAnimator addAnimations:^{ self.transform = CGAffineTransformIdentity; }];
    }
...
@property (nonatomic, strong) NSArray<TOWXV11AvatarCell *> *cells;
...
- (instancetype)initWithFrame:(CGRect)frame {
    ...
    for (NSUInteger i = 0; i < kTOWXV11MaxAvatars; i++) {
        TOWXV11AvatarCell *cell = [[TOWXV11AvatarCell alloc] initWithFrame:CGRectZero];
        cell.avatarIndex = i;
        cell.hidden = YES;
...
- (void)updateCellsAnimated:(BOOL)animated {
    for (NSUInteger i = 0; i < self.cells.count; i++) {
        TOWXV11AvatarCell *cell = self.cells[i];
        ...
        UIImage *image = [value isKindOfClass:[UIImage class]] ? (UIImage *)value : nil;
...
- (void)refreshLayoutPreservingOffset:(BOOL)preserveOffset {
    ...
    if (self.vertical) {
        ...
        for (NSUInteger i = 0; i < self.avatarCount; i++) {
            TOWXV11AvatarCell *cell = self.cells[i];
            cell.frame = CGRectMake(floor((width - diameter) * 0.5), start + i * (diameter + spacing), diameter, diameter);
        }
    } else {
        ...
        for (NSUInteger i = 0; i < self.avatarCount; i++) {
            TOWXV11AvatarCell *cell = self.cells[i];
            cell.frame = CGRectMake(start + i * (diameter + spacing), floor((height - diameter) * 0.5), diameter, diameter);
        }
    }
...
- (void)cellTapped:(TOWXV11AvatarCell *)cell {
    NSUInteger index = cell.avatarIndex;
    if (index >= self.avatarCount) return;
    TOWXV11DiagLog("AVATAR", "TAP|index=%lu|axis=%s", (unsigned long)index, self.vertical ? "vertical" : "horizontal");
    TOWXV11AvatarTapHandler handler = self.tapHandler;
    if (handler) handler(index);
}

- (void)scrollViewWillBeginDragging:(UIScrollView *)scrollView {
    TOWXV11DiagLog("AVATAR", "SCROLL-BEGIN|axis=%s|offset={%.2f,%.2f}", self.vertical ? "vertical" : "horizontal", scrollView.contentOffset.x, scrollView.contentOffset.y);
}
...
__attribute__((constructor)) static void TOWXV11AvatarViewMarker(void) {
    TOWXV11DiagLog("AVATAR", "LOADED|Smooth1-S4|native-scroll+uicontrol");
}
