...
static BOOL TOWXIsWindowVisible(UIWindow *w) { return w && !w.hidden && w.alpha > 0.01 && w.bounds.size.width > 100 && w.bounds.size.height > 100; }
static BOOL TOWXSessionGateVisible(void) {
    for (UIWindow *w in TOWXAllWindows()) if (TOWXIsWindowVisible(w) && [NSStringFromClass(w.class) isEqualToString:TOWX_SESSION_GATE_CLASS]) return YES;
    return NO;
}
...
static BOOL TOWXHostMatches(UIWindow *w) {
    if (!gHasCalibration||!TOWXIsWindowVisible(w)||!gHostWindowClass.length) return NO;
...
static void TOWXAttachToHost(UIWindow *window,const char *mode) {
    if (!window||!TOWXIsWindowVisible(window)) return; CGRect frame=TOWXScaledFrame(window); if (CGRectGetWidth(frame)<160||CGRectGetHeight(frame)<40) return;
...
static BOOL TOWXScanOriginal(void) { for (UIWindow *w in TOWXAllWindows()) if (TOWXIsWindowVisible(w)&&TOWXScanView(w)) return YES; return NO; }
...
static void TOWXEnsureBar(BOOL scanIfNeeded) {
    ...
    if (!gHasCalibration) { if (scanIfNeeded) (void)TOWXScanOriginal(); else (void)TOWXScanOriginal(); if (!gHasCalibration) return; }
    UIScrollView *source=gSourceScroll; if (source&&source.window&&TOWXIsWindowVisible(source.window)) { CGRect f=[source convertRect:source.bounds toView:source.window]; if (CGRectGetWidth(f)>=160&&CGRectGetHeight(f)>=40) { TOWXSuppressOriginalScroll(source); gCalibrationFrameInWindow=f; gCalibrationWindowSize=source.window.bounds.size; gLastHostWindow=source.window; } else { gSourceScroll=nil; TOWXLog("TOWX|SB|P2A4|STALE-SOURCE-DROP|frame={{%.1f,%.1f},{%.1f,%.1f}}",f.origin.x,f.origin.y,f.size.width,f.size.height); } }
...
static void TOWXInstallLifecycle(void) {
    ...
    [c addObserverForName:UIWindowDidBecomeVisibleNotification object:nil queue:NSOperationQueue.mainQueue usingBlock:^(NSNotification *n){TOWXHandleWindowVisible([n.object isKindOfClass:[UIWindow class]]?(UIWindow *)n.object:nil);}];
    [c addObserverForName:UIWindowDidBecomeHiddenNotification object:nil queue:NSOperationQueue.mainQueue usingBlock:^(NSNotification *n){TOWXHandleWindowHidden([n.object isKindOfClass:[UIWindow class]]?(UIWindow *)n.object:nil);}];
