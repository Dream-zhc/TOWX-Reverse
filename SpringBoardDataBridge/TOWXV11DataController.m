...
static int TOWXV11RegisterState(const char *name, int *token) {
    uint32_t status = notify_register_check(name, token);
    if (status != NOTIFY_STATUS_OK) {
        TOWXV11DiagLog("DATA", "STATE-REGISTER-FAIL|name=%s|status=%u", name, status);
        return 0;
    }
    return 1;
}

static BOOL TOWXV11ReadStateToken(int token, uint64_t *value) {
    return notify_get_state(token, value) == NOTIFY_STATUS_OK;
}

static NSString *TOWXV11ResolveExportDir(void) {
    ...
    if (gExportDir.length) {
        TOWXV11DiagLog("DATA", "EXPORT-DIR|path=%s", gExportDir.UTF8String ?: "?");
    } else {
        TOWXV11DiagLog("DATA", "EXPORT-DIR-MISS");
    }
    return gExportDir;
}

static UIImage *TOWXV11DecodeAvatarAtPath(NSString *path) {
    ...
    NSDictionary *options = @{
        ...
        (__bridge NSString *)kCGImageSourceThumbnailMaxPixelSize: @192,
...
static void TOWXV11PublishImagesOnMain(NSArray *images, NSUInteger loaded) {
    dispatch_async(dispatch_get_main_queue(), ^{
        gMainImages = [images copy] ?: @[];
        TOWXV11DiagLog("DATA", "IMAGE-APPLY|count=%lu|loaded=%lu",
                       (unsigned long)gMainCount,
                       (unsigned long)loaded);
        [[NSNotificationCenter defaultCenter] postNotificationName:TOWXV11DataDidChangeNotification object:nil];
    });
}

static void TOWXV11LoadImagesForCount(NSUInteger count) {
    if (!gDataQueue) return;
    dispatch_async(gDataQueue, ^{
        NSString *dir = TOWXV11ResolveExportDir();
        if (!dir.length || count == 0) {
            NSMutableArray *empty = [NSMutableArray arrayWithCapacity:TOWXV11_MAX_RECENTS];
            for (NSUInteger i = 0; i < TOWXV11_MAX_RECENTS; i++) [empty addObject:NSNull.null];
            gDecodedImages = [empty mutableCopy];
            gImageMtimes = [empty mutableCopy];
            TOWXV11PublishImagesOnMain(empty, 0);
            return;
        }

        if (!gDecodedImages) {
            gDecodedImages = [NSMutableArray arrayWithCapacity:TOWXV11_MAX_RECENTS];
            gImageMtimes = [NSMutableArray arrayWithCapacity:TOWXV11_MAX_RECENTS];
            for (NSUInteger i = 0; i < TOWXV11_MAX_RECENTS; i++) {
                [gDecodedImages addObject:NSNull.null];
                [gImageMtimes addObject:NSNull.null];
            }
        }

        NSUInteger limited = MIN(count, (NSUInteger)TOWXV11_MAX_RECENTS);
        NSUInteger loaded = 0;
        for (NSUInteger i = 0; i < TOWXV11_MAX_RECENTS; i++) {
            if (i >= limited) {
            ...
            TOWXV11DiagLog("DATA", "LOAD-BEGIN|index=%lu|path=%s", (unsigned long)i, path.UTF8String ?: "?");
            UIImage *image = TOWXV11DecodeAvatarAtPath(path);
            if (image) {
                gDecodedImages[i] = image;
                gImageMtimes[i] = mtime ?: NSDate.date;
                loaded += 1;
                TOWXV11DiagLog("DATA", "LOAD-END|index=%lu|ok=1|pixels=%.0fx%.0f",
                               (unsigned long)i, image.size.width, image.size.height);
            } else {
                gDecodedImages[i] = NSNull.null;
                gImageMtimes[i] = NSNull.null;
                TOWXV11DiagLog("DATA", "LOAD-END|index=%lu|ok=0", (unsigned long)i);
            }
        }

        TOWXV11PublishImagesOnMain([gDecodedImages copy], loaded);
    });
}

static void TOWXV11ApplyRemoteState(uint64_t generation, uint64_t count, uint64_t stage, uint64_t active, const char *reason) {
    NSUInteger limited = MIN((NSUInteger)count, (NSUInteger)TOWXV11_MAX_RECENTS);
    dispatch_async(dispatch_get_main_queue(), ^{
        ...
        TOWXV11DiagLog("DATA", "STATE|reason=%s|generation=%llu|count=%llu|stage=%llu|active=%d",
                       reason ?: "?",
        ...
        if (gMainCount > 0) TOWXV11LoadImagesForCount(gMainCount);
        else TOWXV11LoadImagesForCount(0);
    });
}

static void TOWXV11RefreshRemoteState(const char *reason) {
    uint64_t generation = 0, count = 0, stage = 0, active = 0;
    if (!TOWXV11ReadStateToken(gGenerationToken, &generation) ||
        !TOWXV11ReadStateToken(gCountToken, &count) ||
        !TOWXV11ReadStateToken(gStageToken, &stage) ||
        !TOWXV11ReadStateToken(gAppActiveToken, &active)) {
        TOWXV11DiagLog("DATA", "STATE-READ-FAIL|reason=%s", reason ?: "?");
        return;
    }
    ...
    gRawStage = stage;
    gRawActive = active;
    if (changed || (reason && strcmp(reason, "ready") == 0)) {
        TOWXV11ApplyRemoteState(generation, count, stage, active, reason);
    }
}
...
void TOWXV11DataSendOpen(NSUInteger index) {
    if (index >= gMainCount || index >= TOWXV11_MAX_RECENTS) return;
    gMainSelectedIndex = (NSInteger)index;
    [[NSNotificationCenter defaultCenter] postNotificationName:TOWXV11DataDidChangeNotification object:nil];
    char name[96];
    snprintf(name, sizeof(name), "%s%lu", TOWX_LINK_OPEN_PREFIX, (unsigned long)index);
    notify_post(name);
    TOWXV11DiagLog("DATA", "OPEN-SEND|index=%lu|notification=%s", (unsigned long)index, name);
}
...
static void TOWXV11StartWatchdog(void) {
    ...
    dispatch_source_set_timer(gWatchdog,
                              ...
                              2 * NSEC_PER_SEC,
                              NSEC_PER_SEC / 5);
    dispatch_source_set_event_handler(gWatchdog, ^{ TOWXV11RefreshRemoteState("watchdog"); });
    dispatch_resume(gWatchdog);
}

__attribute__((constructor)) static void TOWXV11DataControllerInit(void) {
    gMainImages = @[];
    gDataQueue = dispatch_queue_create("com.dream.towx.v11.avatar-data", DISPATCH_QUEUE_SERIAL);
    TOWXV11DiagLog("DATA", "LOADED|Smooth1-S7|background-imageio+2s-watchdog");

    if (!TOWXV11RegisterState(TOWX_LINK_GENERATION, &gGenerationToken) ||
        !TOWXV11RegisterState(TOWX_LINK_COUNT, &gCountToken) ||
        !TOWXV11RegisterState(TOWX_LINK_STAGE, &gStageToken) ||
        !TOWXV11RegisterState(TOWX_LINK_ACK_INDEX, &gAckIndexToken) ||
        !TOWXV11RegisterState(TOWX_LINK_APP_ACTIVE, &gAppActiveToken)) {
        TOWXV11DiagLog("DATA", "INIT-ABORT|state-registration");
        return;
    }

    uint32_t readyStatus = notify_register_dispatch(TOWX_LINK_READY, &gReadyToken, gDataQueue, ^(__unused int token) {
        TOWXV11RefreshRemoteState("ready");
    });
    uint32_t ackStatus = notify_register_dispatch(TOWX_LINK_ACK, &gAckToken, gDataQueue, ^(__unused int token) {
        uint64_t index = UINT64_MAX;
        if (!TOWXV11ReadStateToken(gAckIndexToken, &index)) return;
        dispatch_async(dispatch_get_main_queue(), ^{
            gMainSelectedIndex = index < TOWXV11_MAX_RECENTS ? (NSInteger)index : NSNotFound;
            TOWXV11DiagLog("DATA", "OPEN-ACK|index=%llu", (unsigned long long)index);
            [[NSNotificationCenter defaultCenter] postNotificationName:TOWXV11DataDidChangeNotification object:nil];
        });
    });
    TOWXV11DiagLog("DATA", "LISTENERS|ready=%u|ack=%u", readyStatus, ackStatus);

    dispatch_async(gDataQueue, ^{
        TOWXV11RefreshRemoteState("constructor");
